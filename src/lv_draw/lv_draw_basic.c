...
static uint32_t lv_pic_addr_offset;
extern unsigned char bmp_public_buf[17 * 1024];
//extern void lv_pic_test(uint8_t *P_Rbuff,uint32_t addr,uint32_t size);
extern uint32_t lv_get_pic_addr(uint8_t *Pname);

uint32_t offset_sum,last_offset_sum;
    ...
    if(chroma_key == false && alpha_byte == false && opa == LV_OPA_COVER && recolor_opa == LV_OPA_TRANSP) {
        ...
        /*Normal native VDB*/
			...
			else
			{
				...
				{
					cur_pic.addr_bak = lv_get_pic_addr(&cur_pic.name[0]);
					cur_pic.got_addr = 1;
				}
				cur_pic.addr = cur_pic.addr_bak;
				if(cur_pic.addr == 0)return;
				...
				else
				{
					...
					if(strcmp((const char *)&cur_pic.name[0],(const char *)&cur_pic.name_last[0])!=0)
					{
						//cur_pic.addr += last_offset_sum;
					}
					else
					{
				...
				strcpy((char *)&cur_pic.name_last[0],(const char *)&cur_pic.name[0]);
            ...
            for(row = masked_a.y1; row <= masked_a.y2; row++) {
				...
				{
					lv_pic_test((uint8_t *)vdb_buf_tmp,cur_pic.addr+lv_pic_addr_offset,map_width * px_size_byte);
					//memcpy(vdb_buf_tmp, bmp_public_buf,map_useful_w * sizeof(lv_color_t));
					lv_pic_addr_offset +=map_width * px_size_byte;
				}
#endif
